...
class Note extends FlxSprite
{
	...
	function reloadNote(?prefix:String = '', ?texture:String = '', ?suffix:String = '') {
		...
		if(PlayState.isPixelStage) {
			if(PlayState.curStage != 'RelapseStage')
			{
				...
				if(isSustainNote) {
					loadGraphic(Paths.image('pixelUI/GreyNOTE_assetsENDS'));
					width = width / 4;
					height = height / 2;
					originalHeightForCalcs = height;
					loadGraphic(Paths.image('pixelUI/GreyNOTE_assetsENDS'), true, Math.floor(width), Math.floor(height));
				} else {
			...
			setGraphicSize(Std.int(width *  ClientPrefs.men * PlayState.daPixelZoom));
			loadPixelNoteAnims();
			antialiasing = false;
	...
	function loadNoteAnims() {
		...
		setGraphicSize(Std.int(width * ClientPrefs.men));
		updateHitbox();
	}
