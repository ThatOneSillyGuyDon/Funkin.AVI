...
class PlayState extends MusicBeatState
{
	...
	//The Shaders that aren't in .frag files
	public var camGameShaders:Array<ShaderEffect> = [];
	public var camHUDShaders:Array<ShaderEffect> = [];
	public var camOtherShaders:Array<ShaderEffect> = [];

	var canaddshaders:Bool = ClientPrefs.shaders;
	...
	public var mechanics:Bool = ClientPrefs.mechanics;
	...
	override public function create()
	{
		...
		rating.score = 250;
		rating.noteSplashMarv = false;
		rating.noteSplashSick = true;
		...
		if(isStoryMode)
			...
			Application.current.window.title = "Funkin.avi - Freeplay: " + PlayState.SONG.song + " - Composed by: " + PlayState.SONG.composer;
			...
			switch(hudStyle)
			{
				case 'Psych':
					...
					if (ClientPrefs.marvelouses)
						judgementCounter.text = 'Marvs: ${marvelouss}\nSicks: ${sicks}\nGoods: ${goods}\nBads: ${bads}\nShits: ${shits}\n';
					else
		...
		filmScratchGame.scrollFactor.set();
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
					...
					switch (PlayState.SONG.song)
					{
						...
						case 'Delusional' | 'Twisted Grins' | 'Hunted' | 'Facade' | 'Isolated Old' | 'Mercy' | 'Isolated Legacy' | 'Lunacy Legacy' | 'Mercy Legacy':
							switch (swagCounter)
							{
								case 0:
									countdownBeginning = new FlxSprite().loadGraphic(Paths.image(introAlts[0]));
	...
	public function updateScore(miss:Bool = false)
	{
		switch(hudStyle) {
			...
			case 'Demolition':
				...
				scoreTxt.text = 'Score: ' + songScore
				+ ' | Accuracy: ' + ratingName
				+ (ratingName != '?' ? ' (${Highscore.floorDecimal(ratingPercent * 100, 3)}%) - $ratingFC' : '');
			default:
				scoreTxt.text = 'Score: ' + songScore
				+ ' | Misses: ' + songMisses
				+ ' | Rating: ' + ratingName
		...
		if(ClientPrefs.scoreZoom && !miss && !cpuControlled && hudStyle != 'Vanilla')
		{
	...
	function fadeWhiteFlash(time:Float = 0.3, color:String = '#FFFFFF', ?alphaValue:Float = 0.6) {
		if(ClientPrefs.flashing)
		{
				whiteFlashBG.alpha = alphaValue;
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if(daRating.noteSplashSick && !note.noteSplashDisabled)
		{
			spawnNoteSplashOnNoteSick(note);
		}
		if(daRating.noteSplashMarv && !note.noteSplashDisabled)
		{				
			spawnNoteSplashOnNoteMarvelous(note);
		}
		if(daRating.noteSplashShit && !note.noteSplashDisabled)
		{
		...
		rating.loadGraphic(Paths.image(getUiSkin(uiSkin, daRating.image, altPart)));
...
function ohShitHeGonnaShoot()
{
	...
	public function RecalculateRating(badHit:Bool = false) {
		...
		if (!ClientPrefs.hideJudgement) {
			if (ClientPrefs.marvelouses)
				if(ClientPrefs.language == "Spanish") {
			judgementCounter.text = 'Marav: ${marvelouss}\nExelentes: ${sicks}\nBuenos: ${goods}\nMalos: ${bads}\nTerribles: ${shits}\n';
			} else {
				judgementCounter.text = 'Marvs: ${marvelouss}\nSicks: ${sicks}\nGoods: ${goods}\nBads: ${bads}\nShits: ${shits}\n';
			}
		    else
