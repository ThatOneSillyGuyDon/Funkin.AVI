...
class Cache extends MusicBeatState
{
	...
	override function create()
	{
		...
		for (i in FileSystem.readDirectory(FileSystem.absolutePath("assets/shared/images/funkinAVI")))
			{
				if (!i.endsWith(".png"))
					continue;
				images.push(i);
			...
			for (i in FileSystem.readDirectory(FileSystem.absolutePath("assets/shared/images/funkinAVI-filters")))
				{
					if (!i.endsWith(".png"))
						continue;
					images.push(i);
		...
		sys.thread.Thread.create(() -> { //funi arrow
			cache();
		});
