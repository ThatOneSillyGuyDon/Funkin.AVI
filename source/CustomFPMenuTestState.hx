...
class CustomFPMenuTestState extends MusicBeatState
{
	public var songs:Array<SongMetadataTest> = [];
	...
	private var ostGrp:FlxTypedGroup<OSTArt>;
	...
	override function create()
	{
		...
		bg = new FlxTiledSprite(Paths.image('funkinAVI/fpMenu/creepyWall', 'preload'), FlxG.width * 3, FlxG.width * 3, true, true);
		...
		ostGrp = new FlxTypedGroup<OSTArt>();
		add(ostGrp);
		...
		for (i in 0...songs.length)
		{
			...
			if (songs[i].lockstatus == 'unlocked' || songs[i].lockstatus == 'beaten') 
            {
			    ...
			    ostArtThing.loadGraphic(Paths.image('ostArt/unknownSong', 'preload'));
			}
			ostArtThing.OSTArtTargetY = i;
			ostGrp.add(ostArtThing);
			ostArtThing.scale.set(1.5, 1.5);
			ostArtThing.antialiasing = false;
			ostArtThing.cameras = [camBG];
	...
	{
		songs.push(new SongMetadataTest(songName, weekNum, songCharacter, color, lockstatus));
	}
	...
	function changeSelection(change:Int = 0, playSound:Bool = true)
	{
		...
		var i = 0;
		for(item in ostGrp.members) {
			item.OSTArtTargetY = i - curSelected;
			i++;
		}
...
class OSTArt extends FlxSprite
{

    public var OSTArtTargetY:Int;
    ...
    override function update(elapsed:Float) {
            y = FlxMath.lerp(y, (FlxMath.remapToRange(OSTArtTargetY, 0, 1, 0, 1.3) * 400) + 510, CoolUtil.boundTo(elapsed * 9.6, 0, 1));
            super.update(elapsed);
        } 
